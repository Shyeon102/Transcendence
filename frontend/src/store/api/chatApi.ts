...
export const chatApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getChatRooms: builder.query<ChatRoom[], void>({
      // TODO) 나중에 백엔드 완성되면 query로 갈아끼우기
      // query: () => '/chat/rooms/'
      queryFn() {
      ...
      /* 에러 및 로딩처리 확인용 디버깅 */
      /*queryFn: async () => {
        await new Promise((r) => setTimeout(r, 1000)); // 1초 기다리고
...
export const { useGetChatRoomsQuery } = chatApi; // 훅 노출: 이 훅을 컴포넌트에서 호출하면 데이터/로딩/에러 다 받아짐
