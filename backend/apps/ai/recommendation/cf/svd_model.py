...
from apps.ai.constants import RATING_WEIGHT, ACTION_WEIGHT, MIN_RATINGS_FOR_TRAINING


def build_ratings_df() -> pd.DataFrame:
    from media.models import Review, MediaInteraction

    review_qs = Review.objects.values_list('user_id', 'media_id', 'rating').iterator()
    interaction_qs = MediaInteraction.objects.values_list('user_id', 'media_id', 'action').iterator()
    ...
    if interaction_qs:
        df_int = pd.DataFrame(interaction_qs, columns=['uid', 'iid', 'action_raw'])
    ...
    final_df = total_df.groupby(['uid', 'iid'], as_index=False)['weight'].mean()
...
def train_svd_model(latent_dim=12):
    ...
    if last_model:
        ...
        if current_ratings_count <= last_model_ratings_count:
            print(f"No new ratings since last model. Skipping training.")
            return None

    if current_ratings_count < MIN_RATINGS_FOR_TRAINING:
        print (f"Not enough ratings to train SVD model")
        return None
