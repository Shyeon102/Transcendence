...
def get_hybrid_recommendations(
    ...
    cf_weight, cbf_weight = calculate_sigmoid_weights(
        rating_count=cf_model.user_count)

    cf_series = get_cf_scores(user_id=user_id,
                              exclude_media_ids=exclude_media_ids)
    cbf_series = get_cbf_scores(user_id=user_id, cf_model=cf_model,
                                exclude_media_ids=exclude_media_ids)
